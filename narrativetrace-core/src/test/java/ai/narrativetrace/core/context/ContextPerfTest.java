...
/** Performance test for enter/exit method cycle on ThreadLocalNarrativeContext. */
@Tag("perf")
@ExtendWith(JUnitPerfInterceptor.class)
class ContextPerfTest {
  ...
  private static final JUnitPerfReportingConfig CONFIG =
      JUnitPerfReportingConfig.builder()
          .reportGenerator(new HtmlReportGenerator("build/reports/perf/core-context.html"))
          .build();
  ...
  @JUnitPerfTestRequirement(
      percentiles = PerfThresholds.CORE_ENTER_EXIT_PERCENTILES,
      executionsPerSec = PerfThresholds.CORE_ENTER_EXIT_MIN_OPS,
      allowedErrorPercentage = 0.01f)
  void enterExitCycleMeetsLatencyBudget() {
