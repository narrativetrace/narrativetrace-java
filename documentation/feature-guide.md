...
| Sensitive data redaction — `@NotTraced` (all outputs, always) + name-based redaction rules | Free | `core: NotTraced, RedactionPolicy` | Case-insensitive substring deny-list on field names; built-in defaults (password, token, ssn, …) + custom sets. A type's own `toString()` is never trusted, so every value is walked field by field with redaction consulted per field; only a platform leaf type named in the renderer's own list keeps its own text — a type with no readable field renders as its type name, because state can live off the field graph — and `@NarrativeSummary` is the opt-in back to curated rendering. Map keys render through the same guarded path (redaction + bounds), never raw `toString()`. Single-payload wrappers (`Optional`, the primitive optionals, `Future`, `AtomicReference`) and the list- and pair-shaped holders (`AtomicReferenceArray`, a standalone `Map.Entry`) are opened and their payload rendered by the same rules, so redaction is not lost one container deep |
