...
configuration needed; `narrativetrace.output=false` opts out *(since 0.2.3)*. That default
...
- **The structural `.nt` artifact has no runtime values at all.** Names,
  ...
  name a `name = "…"` template interpolated its arguments into *(since 0.2.3)*. What the
...
- **A trace's name and a run's name carry no data.** Both are a
  ...
  *(since 0.2.3)*) — never from anything captured — so
