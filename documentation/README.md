...
| [Structural Trace Format](structural-trace-format.md) | The value-free `.nt` artifact behind delta reporting and approval traces — the cross-platform format spec |
...
| [The Contract Gate](contract-gate.md) | `documentation/contract.yaml` + `contract-probe/`: proving a documented default, entry point or config shape against a published (never local) install, per-commit schema validation plus a nightly registry-backed check |
