...
`build/narrativetrace` *(since 0.2.2)* — sem nenhuma configuração necessária,
...
| `build/narrativetrace/manifest.json` | Não | Regenerado a cada execução; seu objeto `run` de nível superior (`id`, `name` — a frase de três palavras própria da execução) nomeia *esta execução*, não um cenário, então muda a cada execução mesmo quando mais nada muda *(since 0.2.2)* |
...
| `.claude/skills/**/SKILL.md`, `.agents/skills/**/SKILL.md`, a seção `<!-- narrativetrace:skills:* -->` do `AGENTS.md` | **Sim** *(since 0.2.2)* | Saída de build do catálogo tipado do `narrativetrace-skills`, não saída de uma execução de teste — commitado do mesmo jeito que o `glossary.json`: regenerado, revisado nos diffs, e checado contra deriva (`RenderDriftTest`, ligado ao `./gradlew check`) em vez de editado à mão |
