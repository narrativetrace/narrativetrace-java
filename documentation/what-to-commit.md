...
| `.claude/skills/**/SKILL.md`, the `AGENTS.md` `<!-- narrativetrace:skills:* -->` section | **Yes** *(since 0.2.2, unreleased)* | Build output from `narrativetrace-skills`' typed catalogue, not test-run output — committed the same way `glossary.json` is: regenerated, reviewed in diffs, and checked against drift (`RenderDriftTest`, wired into `./gradlew check`) rather than hand-edited |
