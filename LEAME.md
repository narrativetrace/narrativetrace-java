<!-- source: README.md blob 89678d1ecc65 | translated: 2026-09-14 | reviewed: - -->
# NarrativeTrace
...
`ai.narrativetrace:narrativetrace-core:0.2.2` y todos los módulos que lo
...
|---|---|
| **La ocultación es incondicional** | `@NotTraced` y la lista de denegación por nombre (`password`, `token`, `cvv`, `ssn`, …) se aplican a todas las salidas publicadas — trazas de pruebas, artefactos de CI, logs de contenedor, narración del agente. Ninguna etapa, flag o propiedad las desactiva (decisión del propietario, 2026-08-16). La lista de denegación compara solo por nombre de campo o parámetro, así que una clase que defina tu propio código queda cubierta igual que una integrada — sin anotaciones ni registro. Sobrevive a cualquier envoltorio, a cualquier profundidad (`Optional`, `Future`, `AtomicReference`, `Map.Entry`) y a una clave de `Map` compuesta; el `toString()` propio de un tipo nunca es de fiar mientras el tipo tenga campos, así que uno cuidadosamente escrito no puede imprimir más allá de una ocultación; y una plantilla `{param.ruta}` que nombre un miembro oculto se resuelve como `[REDACTED]`. |
